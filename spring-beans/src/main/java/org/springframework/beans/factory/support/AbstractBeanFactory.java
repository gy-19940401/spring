...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
    ...
    protected <T> T doGetBean(
         ...
         * 原因在于当 bean 对象实现 FactoryBean 接口之后就会变成 &beanName，同时如果存在别名，也需要把别名进行转换
         * */
    ...
    protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
        ...
        // 首先以最小的锁定快速检测并发映射。
        // 从 bean 名称映射到合并的 RootBeanDefinition 的集合中获取 beanName 对应的 RootBeanDefinition
        RootBeanDefinition mbd = this.mergedBeanDefinitions.get(beanName);
        // 如果 mbd 不为 null 且 不需要重新合并定义
    ...
    /**
     * 获取beanName对应的合并 父类后的RootBeanDefinition
     * <p>
     * Return a RootBeanDefinition for the given bean, by merging with the
     * parent if the given bean's definition is a child bean definition.
     *
     * @param beanName     beanDefinition 的名称
     * @param bd           源 beanDefinition
     * @param containingBd ( 如果是内部 bean，则包含 bean 定义，如果是顶层  bean，则为 null )
     * @return 给定bean的一个(可能合并的)RootBeanDefinition
     * @throws BeanDefinitionStoreException in case of an invalid bean definition
     */
    protected RootBeanDefinition getMergedBeanDefinition(
        ...
        synchronized (this.mergedBeanDefinitions) {
            ...
            //该变量表示从bean名称映射到合并的 RootBeanDefinition 集合中取到的mbd且该mbd需要重新合并定义
            RootBeanDefinition previous = null;

            /**
             * Check with full lock now in order to enforce the same merged instance.
             * 立即检查完全锁定，以强制执行相同的合并实例,如果没有包含bean定义
             * containingBd 默认为 null
             */
            if (containingBd == null) {
                // 从 bean 名称映射到合并的RootBeanDefinition集合中获取beanName对应的BeanDefinition作为mbd
                mbd = this.mergedBeanDefinitions.get(beanName);
            }

            // 如果 mbd为null 或者 mdb 需要重新合并定义
            if (mbd == null || mbd.stale) {
                // 将 mdn 赋值给 previous
                previous = mbd;
                // 如果获取不到原始 BeanDefinition 的 父Bean 名
                if (bd.getParentName() == null) {
                    // 没有 父类
                    // Use copy of given root bean definition.
                    // 使用给定的RootBeanDefinition的副本,如果原始BeanDefinition是RootBeanDefinition对象
                    if (bd instanceof RootBeanDefinition) {
                        // 克隆一份 bd 的 Bean 定义赋值给 mdb
                        mbd = ((RootBeanDefinition) bd).cloneBeanDefinition();
                    } else {
                        // 创建一个新的 RootBeanDefinition 作为 bd 的 深拷贝副本 并 赋值给mbd
                        mbd = new RootBeanDefinition(bd);
                    }
                } else {
                    // Child bean definition: needs to be merged with parent.
                    // 父 bean 存在; 子 bean 定义：需要与 父bean 合并,定义一个父级BeanDefinition变量
                    BeanDefinition pbd;
                    try {
                        // 获取bd的父级Bean对应的最终别名 FactoryBean &beanName beanName
                        String parentBeanName = transformedBeanName(bd.getParentName());
                        // 如果当前bean名不等于父级bean名
                        if (!beanName.equals(parentBeanName)) {
                            // TODO 获取parentBeanName的"合并的"BeanDefinition赋值给pdb;{
                            //     找到最顶层的父类？？？
                            //  }
                ...
                // 非单例bean 中包含的 bean 不能是单例。
                // 让我们在此即时进行更正，因为这可能是外部bean的父子合并的结果，在这种情况下，
                // 原始内部bean定义将不会继承合并的外部bean的单例状态。
                // 如果有传包含bean定义且包含bean定义不是单例但mbd又是单例 containingBd 父类 bean
                ...
                if (containingBd == null && isCacheBeanMetadata()) {
                    //将 beanName 和 mbd 的关系添加到 从bean名称映射到合并的RootBeanDefinition集合中
                    // 替换最新的 beanDefinition 到 缓存中
